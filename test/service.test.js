...
beforeAll(() => {
    ...
    return fetch(`${basePath}/catalog/Books(201)`, {
        ...
        .then(response => {
            if(!response.ok) return Promise.reject("setup failed - status code: " + response.status)
        })
})

afterAll(() => {
    // Clean up database - delete all orders created during tests
    let orderIDs = []
    return fetch(`${basePath}/catalog/Orders?$select=ID`)
        .then(response => {
            if(!response.ok) return Promise.reject("Failed to get orders in cleanup")
        ...
        .then(data => {
            ...
            orderIDs.forEach(ID => {
                return fetch(`${basePath}/catalog/Orders(${ID})`, {
                    method: "DELETE"
                })
                    .then(response => {
                        if(!response.status === 204) return Promise.reject(`Failed to delete order ${ID} during cleanup`)
                    })
            })
        })
})
...
describe('Test Domain Model Relationships', () => {
    ...
    it('GET /Orders?$expand=book - responds with all orders with book details', () => {
        return fetch(`${basePath}/catalog/Orders?$expand=book`)
            ...
            .then(data => {
                let targetOrder = data.value.find(order => order.ID === "c13d3eec-942e-470d-97b3-e03322136636")
                expect(targetOrder.ID).toBe("c13d3eec-942e-470d-97b3-e03322136636")
                expect(targetOrder.book_ID).toBe(201)
                expect(targetOrder.book.author_ID).toBe(101)
            })
    })
    ...
    it('GET /Orders?$expand=book($expand=author) - responds with all orders with book and author details', () => {
        return fetch(`${basePath}/catalog/Orders?$expand=book($expand=author)`)
            ...
            .then(data => {
                ...
                expect(targetOrder.ID).toBe("c13d3eec-942e-470d-97b3-e03322136636")
                expect(targetOrder.book_ID).toBe(201)
                expect(targetOrder.book.author.ID).toBe(101)
            })
    })

    it('GET /Books(201)/author - with valid book ID - responds with 200 OK', () => {
        return fetch(`${basePath}/catalog/Books(201)/author`)
            .then(response => {
                expect(response.status).toBe(200)
                return response.json()
            })
            .then(data => {
                expect(data.ID).toBe(101)
                expect(data.name).toBe("Emily Brontë")
            })
    })
    ...
    it('GET /Authors(101)/books - with valid author ID - responds with 200 OK', () => {
        return fetch(`${basePath}/catalog/Authors(101)/books`)
            .then(response => {
                expect(response.status).toBe(200)
                return response.json()
            })
            .then(data => {
                let target = data.value.find(x => x.ID === 201)
                expect(target.ID).toBe(201)
                expect(target.title).toBe("Wuthering Heights")
            })
    })
    ...
    it('GET /Authors(101)/name - with valid author ID - responds with 200 OK', () => {
        return fetch(`${basePath}/catalog/Authors(101)/name`)
            .then(response => {
                expect(response.status).toBe(200)
                return response.json()
            })
            .then(data => {
                expect(data.value).toBe("Emily Brontë")
            })
    })
    ...
    it('GET /Authors(101)/books?$select=title - with valid author ID - responds with 200 OK', () => {
        return fetch(`${basePath}/catalog/Authors(101)/books?$select=title`)
            .then(response => {
                expect(response.status).toBe(200)
                return response.json()
            })
            .then(data => {
                let book = data.value.find(x => x.ID === 201)
